...
type out_update = AddWrite of int * fd * string | RemoveWrite of int
type in_update = AddRead of int * fd | RemoveRead of int
type s = (int * (int * string) list) * (int * int list)
type w = in_update list * out_update list

let zero_w = [], []
let plus_w (inps, outs) (inps', outs') = inps @ inps', outs @ outs'
let with_w w (x, s, w') = x, s, plus_w w w'

type 'a prog = Return of 'a | TryLater of 'a t
...
let rec ( <*> ) f x s =
  ...
  | TryLater f, s, w -> (
    match x s with
    | Return x, s, w' -> TryLater (map_async (( |> ) x) f), s, plus_w w w'
    | TryLater x, s, w' -> TryLater (fun _ -> (f <*> x) s), s, plus_w w w')
...
let read_line fd =
  let rec read = function
    ...
    | (_, inps), outs -> TryLater read, ((0, inps), outs), zero_w
  in
...
let write_line fd str =
  let rec write = function
    ...
    | inps, (pos, outs) -> TryLater write, (inps, (pos + 1, outs)), zero_w
  in
